...
PATCH = ROOT / "recipes-ccsp/rdk-wifi-libhostap/rdk-wifi-libhostap/0006-notify-successful-reauthorization.patch"
...
def test_native_authorization_event_tracks_new_associations(tmp_path, remove_pending_guard):
    ...
    if not compiler:
        pytest.skip("C compiler required for hostap event regression")
...
static bool update_flag(struct Station *sta, int authorized)
{
GUARD
    if (authorized)
...
static void authorize(struct Station *station, int authorized)
{
    if (!update_flag(station, authorized))
        return;
    if (authorized)
        station->connected_events++;
    else
        station->disconnected_events++;
}
int main(void)
{
