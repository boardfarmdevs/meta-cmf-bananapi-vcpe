...
const {expectedFrame, evaluate, distribution, eventPerformance, viewAgreement, recordedEventKind, kernelClientAudit, qualificationFailures} = require('./room-feature-acceptance.js');
...
assert.match(harnessSource, /install -m 0644 \/dev\/stdin \/tmp\/room-feature-rf-audit\.py/);
assert.match(harnessSource, /response\.status\(\) === 409 && \(await response\.json\(\)\)\.error === 'stale_revision'/);
