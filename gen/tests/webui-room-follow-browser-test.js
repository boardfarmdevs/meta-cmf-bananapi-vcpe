...
async function run() {
  ...
  try {
      ...
      if (url.pathname.startsWith('/api/')) {
        ...
        if (url.pathname.endsWith('/room-layout')) {
          ...
          body = {schema: 'easymesh.room-layout.v1', live: true, run_id: 'test', world_epoch: worldEpoch,
            ...
            sequence, state: 'running', observed_at: now, network_observed_at: now,
    ...
    topology.steeringActions = [{sta_mac: station.staMAC, source_bssid: 'source', target_bssid: 'target', method: 'btm-request', requested_at: now}];
