...
clear = re.search(r"        void clear_unassoc_sta_query_msg_id\(\).*?\n        }", header, re.S).group()
program = r'''
#include <arpa/inet.h>
#include <cassert>
#include <cerrno>
...
int main() {
    ...
    radio.clear_unassoc_sta_query_msg_id();
...
'''.replace("CLEAR", clear).replace("SENDER", sender.replace("std::chrono::", "test_clock::"))
