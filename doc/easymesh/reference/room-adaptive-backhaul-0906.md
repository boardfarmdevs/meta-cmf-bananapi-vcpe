...
4. A proposed tree must not reduce any final path score and must improve the summed path score by at least 2 dB per changed parent. One eligible parent changes at a time, prioritizing the largest immediate fleet path-score gain without reducing any intermediate path's RF bottleneck. The original 4 dB threshold is reduced for 0907: shifted Home B's upper-right `extender_2` role at `(17,3)` has a direct score of 20, but reaches 22 through `extender_1` at `(10,2)` (`min(31, 25) - 3`). Resolve display names from the current role/MAC mapping; their numbering need not match role suffixes. Keeping the weaker direct route also exposed its documented large-packet delivery limitation. Ties and 1 dB improvements still do not trigger changes; the existing settling, cooldown, failure and rollback guards remain. The status API reports `minimum_tree_gain_db_per_change` explicitly.
...
7. Allow up to 30 seconds to verify the real `wifi1.3` association and two consecutive gateway probes over `brlan0`. Transient link-query failures reset the success streak and retry within that window instead of immediately rolling back a valid association. On failure, restore and verify the previous BSSID. Individual subprocesses have timeouts. The viewer can briefly wait behind this serialized transaction; lease time is extended by the internal action duration so a browser is not evicted while renewals wait. Controller topology may lag the kernel association.
