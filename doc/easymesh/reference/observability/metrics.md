...
[nested LXD UI and Prometheus/Grafana monitoring](monitoring.md).
Those resource metrics are separate from the EasyMesh radio reports below.

## Current state

Agents send AP Metrics Responses on their configured interval. The controller
stores the STA, BSS and radio portions; collection health and freshness must
still be checked for the current deployment.
...
Open the controller WebUI from [current state](../../current-state.md) and
select **Policy Settings**. Use the actual forwarded address for your deployment.
